...
public class QuestionLibraryController {
	...
	public ModelAndView createQuestion(QuestionDto questionDto) {
		...
		ModelAndView modelAndView=new ModelAndView(questionLibraryOperationStatus);
		try {
			Question savedQuestion = questionLibraryServiceImpl.createQuestion(question);
	...
	public ModelAndView updateDifficultyLevel(@RequestParam("id") int id, @RequestParam("difficultyLevel") String difficultyLevel) {
		ModelAndView modelAndView=new ModelAndView(updationStatus);
		Question updatedQuestion;
		try {
		updatedQuestion = questionLibraryServiceImpl.updateDifficulty(id, difficultyLevel);
		format = String.format("Question difficultyLevel with id: %d updated successfully", updatedQuestion.getId());
		modelAndView.addObject(message, format);
		}
		catch(QuestionNotFoundException e)
		{
	...
	public ModelAndView updateOption(@RequestParam("id") int id,@RequestParam("index") int index, @RequestParam("value") String value) {
		ModelAndView modelAndView=new ModelAndView(updationStatus);
