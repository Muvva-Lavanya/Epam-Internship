...
@ExtendWith(MockitoExtension.class)
 class UserDaoImplTest {
	...
	void testAllUsers()
	{
		...
		Mockito.when(entityManager.createQuery("select user from User user",User.class)).thenReturn(query);
	...
	void testCheckUser()
	{
		...
		Mockito.when(entityManager.createQuery(queryString,User.class)).thenReturn(query);
		Mockito.when(query.getResultList()).thenReturn(usersList);
		boolean result=userDaoJpaImpl.checkUser(user);
		Mockito.verify(entityManager).createQuery(queryString, User.class);
		Mockito.verify(query).getResultList();
		assertEquals(true, result);
		
	}
	@Test
	void testCheckNotUser()
	{
		User user=new User("user","Lavanya","Lavanya@123"); 
		List<User> usersList=new ArrayList<>();
		//usersList.add(user);
