...
import com.epam.restapi.QuizRestController;
...
class QuizRestControllerTest {
	...
	void testCreateQuiz() throws Exception {
		...
		mockMvc.perform(post("/quizzes")
               ...
               .content(new ObjectMapper().writeValueAsString(quizDto)).with(csrf()))
	...
	void testDeleteQuiz() throws Exception {
		...
		mockMvc.perform(delete("/quizzes/{id}", 1).with(csrf())).andExpect(status().isNoContent()).andReturn();
	}
