...
class QuizLibraryControllerTest {
	...
	{
		List<Integer> questionIds=Arrays.asList(1);
		Mockito.when(quizLibraryServiceImpl.updateQuiz(any(), eq(questionIds))).thenThrow(new QuestionException("Quiz already exists"));
		mockMvc.perform(post("/updateQuiz").param("questionIds","1")).andExpect(status().isOk())
		.andExpect(view().name("quizLibraryOperationStatus.jsp")).andExpect(model().attributeExists("message"))
	...
	{
		List<Integer> questionIds=Arrays.asList(1);
		Mockito.when(quizLibraryServiceImpl.addQuiz(any(), eq(questionIds))).thenThrow(new QuizException("Quiz already exists"));
	...
	void testViewQuizById() throws Exception
	{
		...
		.andExpect(model().attribute("quiz",quizDto)).andReturn();
